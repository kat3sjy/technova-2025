...
function fallbackAnalysis(user) {
...
export async function analyzeProfile(user) {
    if (!genAI)
...
export async function scoreCompatibility(a, b) {
    ...
    const reasons = [];
    if (areasScore > 0)
        reasons.push(`Shared areas overlap ${(areasScore * 100).toFixed(0)}%`);
    if (interestsScore > 0)
        reasons.push(`Interest similarity ${(interestsScore * 100).toFixed(0)}%`);
    if (criteriaScore > 0)
        reasons.push(`Match criteria alignment ${(criteriaScore * 100).toFixed(0)}%`);
    if (reasons.length === 0)
        reasons.push('Limited overlap inferred from profiles.');
